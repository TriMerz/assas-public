...
def create_performance_comparison_plots(embedder, original_data, reconstructed_data):
    ...
    for var_name, indices in feature_types.items():
        ...
        # Calculate R² and correlation for valid features
        r2_scores = []
        correlations = []
        
        for idx in indices:
            if idx < original_data.shape[1] and np.std(original_data[:, idx]) > 1e-10:
                r2 = r2_score(original_data[:, idx], reconstructed_data[:, idx])
                corr = np.corrcoef(original_data[:, idx], reconstructed_data[:, idx])[0, 1]
        ...
        metrics_data.append([
            ...
            f"{mse:.6f}",
            f"{mae:.6f}",
            f"{np.mean(r2_scores):.3f}" if r2_scores else "N/A",
            f"{np.mean(correlations):.3f}" if correlations else "N/A",
            str(len(indices))
        ])
