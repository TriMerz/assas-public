...
class NonLinearEmbedder:
    ...
    def evaluate_reconstruction(self, windows, n_samples=5):
        ...
        Also plots a scatter plot of predicted vs measured values.
        """
        ...
        for i, feat_idx in enumerate(feature_indices):
            ...
            # Calculate metrics
            y_true = original_series[:, feat_idx]
            y_pred = reconstructed_series[:, feat_idx]
            abs_error = np.abs(y_true - y_pred)
            abs_perc_error = abs_error / (np.abs(y_true) + 1e-8)
            ...
            smape = 100 * np.mean(2 * abs_error / (np.abs(y_true) + np.abs(y_pred) + 1e-8))
        ...
        print("\nPlotting scatter of predicted vs measured values...")
        scatter_samples = min(10000, total_length * n_samples)  # Limit for visibility
        all_measured = []
        all_predicted = []
        for feat_idx in feature_indices:
            all_measured.append(original_series[:, feat_idx])
            all_predicted.append(reconstructed_series[:, feat_idx])
        all_measured = np.concatenate(all_measured)
        all_predicted = np.concatenate(all_predicted)
        # Random subsample for large arrays
        if len(all_measured) > scatter_samples:
            idx = np.random.choice(len(all_measured), scatter_samples, replace=False)
            all_measured = all_measured[idx]
            all_predicted = all_predicted[idx]
        plt.figure(figsize=(8, 8))
        plt.scatter(all_measured, all_predicted, alpha=0.3, s=8, color='purple')
        min_val = min(all_measured.min(), all_predicted.min())
        max_val = max(all_measured.max(), all_predicted.max())
        plt.plot([min_val, max_val], [min_val, max_val], 'k--', lw=2, label='Ideal')
        plt.xlabel('Measured')
        plt.ylabel('Predicted')
        plt.title('Predicted vs Measured Scatter Plot')
        plt.legend()
        plt.grid(True, alpha=0.3)
        scatter_path = self.checkpoint_dir / 'predicted_vs_measured_scatter.png'
